...
from data_process.dataset.hrfa_three_way_dataset import HrfaThreeWayDataset
...
class HrfaThreeWayModel(BaseModel):
    def __init__(self, config: HrfaConfig, word_embedding_weight):
        ...
        self.dataset_class = HrfaThreeWayDataset
    ...
    def forward(self, user_id, user_review, item_id, item_review,
        ...
        predict = self.fm_layer(torch.cat([user_comb_latent, item_latent], dim=1))
        return predict
...
def test():
    ...
    model = HrfaThreeWayModel(config, word_dict.weight)
