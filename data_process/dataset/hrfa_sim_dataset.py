...
class HrfaSimDataset(BaseDataset):
 ...
 \
                  torch.LongTensor(data.user_pos_review[index].copy()), \
                  torch.LongTensor(data.item_pos_review[index].copy()), \
 \
...
def test():
    dataset = HrfaSimDataset(DataSetEnum.Appliances, DataTypeEnum.Train)
